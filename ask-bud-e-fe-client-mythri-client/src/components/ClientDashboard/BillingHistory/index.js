  ...
  useEffect(() => {
    ...
    const fetchBillingHistory = async () => {
      try {
        ...
        if (!res.ok) {
          throw new Error(`HTTP ${res.status}: Failed to fetch billing history`);
        }

        const json = await res.json();
        
        if (!json.success) {
          throw new Error(json.message || "Failed to fetch billing history");
        }

        // Set the billing history data
        setRows(Array.isArray(json.data) ? json.data : []);
        
      } catch (error) {
        if (error.name !== 'AbortError') {
          console.error("[BillingHistoryList] Fetch error:", error);
